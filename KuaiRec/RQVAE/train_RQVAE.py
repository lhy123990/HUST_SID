...
def train_rqvae(args):
    ...
    dataset = SCLDataset(
        feature_map_path,
        keys_filename=args.keys_filename,
        values_filename=args.values_filename,
        normalize=not args.no_normalize,
    )
    dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
    # Validation/Stats loader without shuffle
    stats_loader = DataLoader(dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
    ...
    # 4. 生成并保�????????? Semantic IDs
    save_semantic_ids(model, dataset, args.output_dir, device, args.batch_size, args.num_workers)

def save_semantic_ids(model, dataset, output_dir, device, batch_size, num_workers):
    model.eval()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default='/data/cbn01/KuaiRec/data/bge_m3_caption_embeddings')
    parser.add_argument('--output_dir', type=str, default='/data/cbn01/KuaiRec/data/bge_m3_caption_embeddings')
    parser.add_argument('--keys_filename', type=str, default='video_ids.npy')
    parser.add_argument('--values_filename', type=str, default='bge_m3_embeddings.npy')
    parser.add_argument('--no_normalize', action='store_true', default=False)
    parser.add_argument('--model_name', type=str, default='rqvae_model.pth')
    ...
    parser.add_argument('--codebook_structure', type=int, nargs='+', default=[64,64,64])
    parser.add_argument('--mlp_dim',  type=int, nargs='+', default=[256,128], help="SIDTierMLPDimension")
